...
export default function EnterpriseSsoPage() {
  ...
  const loadConnections = useCallback(async (clientId: string) => {
    ...
    const [error, data] = await fetchApi<ConnectionType[]>(
      `/v1/connections/types?clientId=${clientId}&social=false`
    )
    if (error) {
      setErrorMsg(error.message)
    } else {
    ...
    <div className="flex-1 space-y-6 p-6 md:p-8">
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage SSO connections (SAML/OIDC) per application.
          </p>
        </div>
      ...
      {!hasApplications && !loading && (
        <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">
          Create an application to manage SSO connections.
        </div>
      )}
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="rounded-lg border bg-card p-6 text-sm text-muted-foreground">
            No SSO connections available yet.
          </div>
        ) : (
                ...
                {fields.length === 0 ? (
                  <div className="space-y-3">
                    <div className="rounded-md border bg-muted/30 p-3 text-xs text-muted-foreground">
                      SSO configuration schema will appear here once available.
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-1">
                        <Label className="text-xs">SAML Metadata URL</Label>
                        <Input disabled placeholder="https://idp.example.com/metadata" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Entity ID</Label>
                        <Input disabled placeholder="urn:authora:tenant" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">ACS URL</Label>
                        <Input disabled placeholder="https://yourapp.com/sso/callback" />
                      </div>
                    </div>
                  </div>
                ) : (
